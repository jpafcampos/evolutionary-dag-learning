...
def MAGA_adpt(population, max_eval_bic, Pm_min, Pm_max, Po, Pc_min, Pc_max, t_max, goal_bic, sL, sPm, sGen, file, self_learn, feasible_only, verbose=False):
    local_search_threshold = max_eval_bic // 2
    best_bic = float('inf')
    best_graph = None
    best_pos = 0
    iteration = 0
    ...
    while (best_bic > goal_bic + 0.00000001) and (num_eval_bic < max_eval_bic):
        ...
        if self_learn and num_eval_bic > local_search_threshold:
            if verbose:
...
def compute_metrics():
    ...
    filename = PATH +f'MAGA_adpt_all_runs_results_{args.data}.csv'
...
if __name__ == '__main__':
    ...
    PATH = '/home/joao/Desktop/UFMG/PhD/code/EA-DAG/results/MAGA_adpt/' + args.data + '/'
    # if PATH does not exist, change it to the path in the server
    if not os.path.exists(PATH):
        PATH = '/home/joaocampos/phd_code/evolutionary-dag-learning/results/MAGA_adpt/' + args.data + '/'
    if not os.path.exists(PATH):
        PATH = '/home/bessani/phd_code/evolutionary-dag-learning/results/MAGA_adpt/' + args.data + '/'
    ...
    for i in range(n_runs):
        ...
        best_graph, num_eval_bic, population, reached_goal = MAGA_adpt(population, max_bic_eval, Pm_min, Pm_max, Po, Pc_min,
